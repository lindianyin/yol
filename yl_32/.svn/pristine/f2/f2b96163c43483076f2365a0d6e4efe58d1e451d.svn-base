...
template<
    class TKey, 
    class TValue, 
    class TEraseFunc = DefaultEraseFunc<TKey, TValue> 
>
class KLRUCahceMap
{
public:
...
BOOL KLRUCahceMap<TKey, TValue, TEraseFunc>::Init(int nCapacity, int nRecycleSize)
{
    ...
    PROCESS_ERROR(nCapacity    > 0);
    PROCESS_ERROR(nRecycleSize > 0);
    ...
    PROCESS_ERROR(m_pPreAllocatedNodes);
...
EXIT0:
    if (!bResult)
    {
        SAFE_DELETE_ARRAY(m_pPreAllocatedNodes);
    }
    return bResult;
}
...
TValue* KLRUCahceMap<TKey, TValue, TEraseFunc>::Insert(const TKey& crKey)
{
    ...
    LOG_PROCESS_ERROR(nLeftCapacity > 0);
    ...
    LOG_PROCESS_ERROR(RetPair.second);
...
BOOL KLRUCahceMap<TKey, TValue, TEraseFunc>::Erase(const TKey& crKey)
{
    ...
    it = m_CacheMap.find(crKey);
    PROCESS_ERROR(it != m_CacheMap.end());
...
BOOL KLRUCahceMap<TKey, TValue, TEraseFunc>::Clear()
{
    ...
    LOG_PROCESS_ERROR(uSize == uErase);
...
int KLRUCahceMap<TKey, TValue, TEraseFunc>::_Recycle()
{
    ...
    while (nCount < m_nRecycleSize)
    {
        ...
        PROCESS_ERROR(m_LRUListEnd.pPrevLRUNode != &m_LRUListHead);
