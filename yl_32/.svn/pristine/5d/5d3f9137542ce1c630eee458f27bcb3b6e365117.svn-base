...
local tb	= Task:GetTarget("HavePetNum");
tb.szTargetName	= "HavePetNum";
tb.REFRESH_FRAME	= 18;	-- 一秒检测一次

function tb:Init(szStaticDesc, szDynamicDesc, nTargetPetNum)
	self.szStaticDesc	= szStaticDesc;
	self.szDynamicDesc	= szDynamicDesc;
	self.nTargetPetNum = nTargetPetNum
end;

function tb:Start()
	self.bDone = 1;
	local tbPets = KPet.GetPlayerPetList(self.me)
	if (#tbPets < self.nTargetPetNum) then
	   self.bDone = 0
	end
