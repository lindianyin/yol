...
-------------------------------------------------------
local tb = Task:GetTarget("FinishTask")

tb.szTargetName = "FinishTask"

-- dialognpc, number, number, txt, txt
function tb:Init(nNpcTempId, nTaskId, nNeedCount, szStaticDesc, szDynamicDesc)
...
function tb:OnFinishTask(pPlayer, nTaskId)
	print("...FinishTask receive task finish event！！！！！")
	if (nTaskId == self.nTaskId) then
		...
		if (self:IsDone()) then	-- 本目标是一旦达成后不会失效的
			self.me.SysMsg(Lang.task.target.str27[Lang.Idx]..self:GetStaticDesc());
