...
BOOL KMailCountNotifier::NotifyMailCount(INT64 nMailCount)
{
	...
	else
	{
		...
		else
		{
			...
			KGcPlayerCommon* pPlayer = pPlayerMgr->GetPlayerById(m_sInfo.dwReceiverId);
										  ...
										  KItemData_Common* pItemData, INT nItemCount, INT nIconId)
{
	...
	KGcPlayerCommon  * pReceiver = m_pIKOnlinePlayerMgr->GetPlayerById(dwReceiverId);
...
BOOL KGC_MailProtocalHandler::FetchMailList(IKEnumMail *pEnum, DWORD dwPlayerId, INT nConnectId)
{
	...
	*pHead = KMAIL_G2S_PROTO_FETCHINFO();
	...
	while (pEnum->NextMail(&pMail) && nCount < nMailCount)
	{
		...
		KGcPlayerCommon* pReceiver = m_pIKOnlinePlayerMgr->GetPlayerById(pMailHead->dwReceiverId);
		if (!pReceiver)
		{
...
BOOL KGC_MailProtocalHandler::Process_FetchItem(LPBYTE pData, UINT uSize, INT nConnectId)
{
	...
	else
	{
		if (KMailPlayerManager::Instance()->GetMailLoaded(pMsg->dwPlayerId) == 1)
		{
			KGcPlayerCommon* pPlayer = m_pIKOnlinePlayerMgr->GetPlayerById(pMsg->dwPlayerId);
			QCONFIRM_RET_FALSE(pPlayer);
...
BOOL KGC_MailProtocalHandler::Process_CheckReceiver(LPBYTE pData, UINT uSize, INT nConnectId)
{
	...
	for (INT i = 0; i < nReceiverCount; ++i)
	{
		...
		// 判断是否存在该名字的玩家
		KGcPlayerCommon* pPlayer = m_pIKOnlinePlayerMgr->GetPlayerByName(szReceiverName);
		if (!pPlayer)
		{
			bReceiverOk = FALSE;
...
BOOL KGC_MailProtocalHandler::SendMail(CONST KMAIL_S2G_PROTO_SENDMAIL *pMsg, KMAIL_INFO *pMail)
{
	...
	for (INT i = 0; i < pMsg->wReceiverCount; ++i, pdwReceiverId++)
	{
		...
		KGcPlayerCommon* pReceiver = m_pIKOnlinePlayerMgr->GetPlayerById(*pdwReceiverId);
		if (!pReceiver)
		{
...
BOOL KGC_MailProtocalHandler::SendPlayerMail(DWORD dwSenderId, KGcPlayerCommon* pReceiver, KMAIL_INFO* pMail)
{
	QCONFIRM_RET_FALSE(pReceiver && pMail);

	KGcPlayerCommon* pSender = m_pIKOnlinePlayerMgr->GetPlayerById(dwSenderId);
	if (!pSender)
	{
...
BOOL KGC_MailProtocalHandler::SendSystemMail(KGcPlayerCommon* pReceiver, KMAIL_INFO* pMail)
{
	QCONFIRM_RET_FALSE(pReceiver && pMail);
...
BOOL KGC_MailProtocalHandler::SendDeliveryMail(DWORD dwSenderId, KGcPlayerCommon* pReceiver, KMAIL_INFO* pMail)
{
	QCONFIRM_RET_FALSE(pReceiver && pMail);
