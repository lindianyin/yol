...
{
    if (m_nRefreshCycle > 0 && g_pGameCenter->m_nTimeNow > m_nNextRefreshTime)
    {
        Refresh();
        m_nNextRefreshTime = (g_pGameCenter->m_nTimeNow / m_nRefreshCycle + 1) * m_nRefreshCycle;
    }
    ...
    for (KMAP_COPY_TABLE::iterator it = m_CopyTable.begin(); it != m_CopyTable.end(); NULL)
    {
        ...
        {
            g_pGameCenter->m_MiscDB.DoSaveMapCopyInfo(m_dwMapID, pMapCopy->m_nCopyIndex);
            pMapCopy->m_nLastSaveTime = g_pGameCenter->m_nTimeNow;
        ...
        if (pMapCopy->m_bAutoDelete && pMapCopy->m_eState == eMapStateNormal)
        {
            if (pMapCopy->m_nUnloadTime == 0)
            {
                if (pMapCopy->m_nPlayerCount < m_nMinPlayerCount)
                {
                    ...
                    pMapCopy->m_nUnloadTime = g_pGameCenter->m_nTimeNow + m_nKeepTime * 60;
                }
            }
            else
            {
                if (g_pGameCenter->m_nTimeNow > pMapCopy->m_nUnloadTime)
                {
                    g_pGameCenter->m_GameServer.DoMapCopyDoClearPlayer(pMapCopy->m_nConnIndex, m_dwMapID, it->first);
                ...
                {
                    if (g_pGameCenter->m_nWorkLoop % (3 * GAME_FPS) == 0)
                    {
                        time_t nKeepTime = pMapCopy->m_nUnloadTime - g_pGameCenter->m_nTimeNow;
        ...
        {
            if (g_pGameCenter->m_nTimeNow > pMapCopy->m_nRefreshTime)
            {
                g_pGameCenter->m_GameServer.DoMapCopyDoClearPlayer(pMapCopy->m_nConnIndex, m_dwMapID, it->first);
            ...
            {
                g_pGameCenter->m_GameServer.DoDeleteMapNotify(pMapCopy->m_nConnIndex, m_dwMapID, it->first);

                g_pGameCenter->m_MiscDB.DoDeleteMapCopyInfo(m_dwMapID, it->first);
...
)
{
    ...
    CopyInfo.m_dwOwnerID              = KD_BAD_ID;
...
void KMapInfo::DeleteAllCopyOnConnection(int nConnection)
{
    for (KMAP_COPY_TABLE::iterator it = m_CopyTable.begin(); it != m_CopyTable.end(); NULL)
    {
        ...
        {
            QLogPrintf(
                LOG_INFO, "Delete map copy %s(%u, %d)\n", 
...
DWORD KMapInfo::GetNewOwnerForPlayerEnter(KRole* pRole, int nCopyIndex)
{
    ...
    PROCESS_ERROR(m_nType == emtDungeon);

    pMapCopy = GetMapCopy(nCopyIndex);
    LOG_PROCESS_ERROR(pMapCopy);

    if (pMapCopy->m_dwOwnerID != KD_BAD_ID)
    {
        pOldOwner = g_pGameCenter->m_RoleManager.GetRole(pMapCopy->m_dwOwnerID);
    }
    ...
    pTeam = g_pGameCenter->m_TeamCenter.GetTeam(pRole->m_dwTeamID);
...
BOOL KMapInfo::CheckPlayerEnterNewCopyTimes(DWORD dwPlayerID, int nNewCopyIndex)
{
    ...
    PROCESS_SUCCESS(it->second.nCopyIndex == nNewCopyIndex); // 如果进的是同一个副本则允许进入
    PROCESS_ERROR(it->second.nTimes < m_nLimitTimes); // 超过场景的进入次数限制则不允许进入
