	...
	default:
		...
		// flags |= SIMPLE at the end.
		{
			...
			for ( regprev = 0 ; ; ) {
				...
				// special treatment for Chinese GBK
				if ((_TUCHAR)ch > 0x80)
				{
					regc(ch);
					ch = *regparse++;
			...
			case ANYOF:
				{
					if (! *reginput)
						return false;
					char* pPattern = OPERAND(scan);
					WORD wSearch = 0;
					if (*(BYTE*)reginput > 0x80)
						wSearch = *(WORD*)reginput;
					else
						wSearch = *reginput;
					int nFound = FALSE;
					while (*pPattern)
					{
						if (*(BYTE*)pPattern > 0x80)
						{
							if (wSearch == *(WORD*)pPattern)
							{
								nFound = TRUE;
								break;;
							}
				...
				//if (*reginput == '\0' ||
				//		_tcschr((BYTE*)OPERAND(scan), *reginput) == NULL)
				//	return false;
				//reginput++;
				break;
			case ANYBUT:
...
const CString Regexp::operator[]( unsigned int i ) const
{
	...
	if ( rc )
	{
		...
		TCHAR* szbuf = (TCHAR*)malloc(buflen);
...
CString regexp::GetReplaceString( const TCHAR* sReplaceExp ) const
{
	...
	size_t reservesize = (size_t)(repsize < (size_t)replacelen ? repsize : (size_t)replacelen);
