...
local tb	= Task:GetTarget("ComposePet");
tb.szTargetName	= "ComposePet";
tb.REFRESH_FRAME	= 18;	-- 一秒检测一次

function tb:Init(szStaticDesc, szDynamicDesc, nComposeNum)
	self.szStaticDesc	= szStaticDesc;
	self.szDynamicDesc	= szDynamicDesc;
	self.nComposeNum = nComposeNum;
end;

function tb:Start()
	self.bDone = 1;
	local nComposeCount = self.me.pTask.GetTask(5001, 2)
	if nComposeCount < self.nComposeNum then
	    self.bDone = 0
	end
