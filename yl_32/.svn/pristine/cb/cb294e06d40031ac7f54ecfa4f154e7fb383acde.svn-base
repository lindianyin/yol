...
local tb	= Task:GetTarget("FinishedTeamLevel");
tb.szTargetName	= "FinishedTeamLevel";
tb.REFRESH_FRAME	= 18;	-- 一秒检测一次

function tb:Init(szStaticDesc, szDynamicDesc, nDifficulty)
...
function tb:Start()
	self.bDone = 1;
	local nStartNum = KStage.GetStageStar(self.me, self.nDifficulty)
	if nStartNum < 2 then
	    self.bDone = 0;
	end
