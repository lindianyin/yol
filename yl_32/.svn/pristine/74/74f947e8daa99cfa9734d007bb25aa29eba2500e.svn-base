-------------------------------------------------------
-- 文件名　：upgradeequip.lua
-- 文件描述：升级装备
-- 创建者　：zhangzhixiong
-- 创建时间：2014-3-27
-------------------------------------------------------

local tb	= Task:GetTarget("HaveEquipNum");
tb.szTargetName	= "HaveEquipNum";
tb.REFRESH_FRAME	= 18;	-- 一秒检测一次

function tb:Init(szStaticDesc, szDynamicDesc, nTargetLevel, nTargetEquipNum)
	self.szStaticDesc	= szStaticDesc;
	self.szDynamicDesc	= szDynamicDesc;
	self.nTargetLevel = nTargetLevel;
	self.nTargetEquipNum = nTargetEquipNum
end

function tb:Start()
	self.bDone = 1;
	local nEquipNum = Item:GetEquipNumByLevel(me, self.nTargetLevel);
	if nEquipNum < self.nTargetEquipNum then
	    self.bDone = 0;
	end
