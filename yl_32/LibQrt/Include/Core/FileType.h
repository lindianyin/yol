...
public:
	enum	INIFILE_PARAM
	{
		INI_MAX_SUPPORT_MULTI_VALUE_COUNT   = 256,	//通过GetMulti*,WriteMulti*函数操作的值的上限数目，操出限制返回失败
...
CORE_API_C	IIniFile*	g_CreateIniFile();
CORE_API_C	ITabFile*	g_CreateTabFile();
CORE_API_C	IIniFile*	g_OpenIniFile(const char* FileName, int ForceUnpakFile = false, int ForWrite = false);
CORE_API_C	ITabFile*	g_OpenTabFile(const char* FileName, int ForceUnpakFile = false, int ForWrite = false);
CORE_API bool	g_OpenFile(IIniFile** ppIniFile, const char* FileName, int ForceUnpakFile = false, int ForWrite = false);
CORE_API bool	g_OpenFile(ITabFile** ppTabFile, const char* FileName, int ForceUnpakFile = false, int ForWrite = false);
...
CORE_API IIniFile *g_OpenIniFileFromArguments(int argc, char* argv[]);
...
// Return a read only ini file interface
CORE_API IIniFile *g_OpenCombinatorialIniFile(IIniFile* piHighPriority, IIniFile* piLowPriority);

////////////////////////////////////////////////////////////////////////////////
// Load start arguments from command line and an ini file, and command arguments
// have high priority
// Return a read only ini file interface
CORE_API IIniFile *g_LoadStartArgument(int argc, char *argv[], const char *FileName);
