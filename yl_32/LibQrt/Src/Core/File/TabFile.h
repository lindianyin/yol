...
class QTabFileImpl : public ITabFile
{
public:	//ITabFile接口函数
	void	Clear();
	void	Release();
	...
	//下列函数在KTabFileImpl中不支持，都直接返回失败(false/0)
	...
	int			InsertAfter(const char* szRow, int bColumnLab = true) _NOT_IMPL_;
	int			InsertBefore(int nRow) _NOT_IMPL_;
	int			InsertBefore(const char* szRow, int bColumnLab = true) _NOT_IMPL_;
...
public:
	QTabFileImpl();
	virtual ~QTabFileImpl();
	//构造一个空的KTabFileImpl对象
	static	QTabFileImpl* New();
