...
static QLOG_PARAM      gs_LogParam = { 0 };
...
int QLogInit(const QLOG_PARAM& cLogParam, void *pvContext)
{
    int nResult  = false;
    int nRetCode = false;
    ...
    if (gs_LogParam.Options == 0)
        gs_LogParam.Options =(QLOG_OPTIONS)(LOG_OPTION_FILE | LOG_OPTION_CONSOLE);
...
int QLogSetPriorityMask(int nPriorityMask)
{
//	if (nPriorityMask != 0)
...
int QLogPrintf(QLOG_LEVEL Priority, const char cszFormat[], ...)
{
    if ((QLOG_MASK(QLOG_PRI(Priority)) & gs_nPriorityMask) == 0)
        return true;
	...
	nRetCode = snprintf(
        ...
        gs_caszPriorityString[QLOG_PRI(Priority)],
        QGetCurrentThreadId()
    );
    ...
    szBuffer[sizeof(szBuffer) - 1] = '\0';
	 ...
	 // bugfix: linux buffer overflow (glibc 2.1以后vsnprintf遵循C99，返回值与vc2005不同) [12/9/2011 simon]
