...
local tbNpc = Npc:GetClass("battlefield_wuhunju")
...
function tbNpc:GoToActivity(nActivityGroup,nIndex,nAlreadyStart)
	...
	local tbOpt = {
		{Lang.npc.str19[Lang.Idx],self.GoToActivity2,self,nActivityGroup,nIndex,nAlreadyStart,nPlayerNum2},
...
function tbNpc:GoToActivity2(nActivityGroup,nIndex,nAlreadyStart,nPlayerNum)
	...
	if nAlreadyStart == 0 then
		Activity:JoinActivity(me.dwId, nActivityTarget, WUHUNJUSTEP);
	else
		if nPlayerNum >= 20 then
			Dialog:Say(Lang.npc.str24[Lang.Idx]);
		else
