    ...
    LOCK =
    {
       -- 1号锁不能不填，默认1号为起始锁
		[1] = {nTime = 1, nNum = 0,		-- 总计时
			...
			tbStartEvent =
			{
			    {MultiInstance.TIME_INFO, Lang.mission.str1187[Lang.Idx]},
				{MultiInstance.REVIVE_INFO, true, true, false},
				{MultiInstance.SHOW_NOTICE_MSG, Lang.mission.str1188[Lang.Idx]},
				{MultiInstance.TARGET_INFO, Lang.mission.str1189[Lang.Idx], 1},
			},
			tbUnLockEvent =
			{
			},
		...
		[2] = {nTime = 0, nNum = 1 ,		-- 锁通过的条件（时间倒计时，杀怪个数）
			...
			tbStartEvent =
			{
				...
				{MultiInstance.TARGET_INFO, Lang.mission.str1190[Lang.Idx], 1},
			},
			tbUnLockEvent =
			{
