    ...
    LOCK =
    {
       -- 1号锁不能不填，默认1号为起始锁
		[1] = {nTime = 1, nNum = 0,		-- 总计时
			...
			tbStartEvent =
			{
			    {MultiInstance.TIME_INFO, Lang.mission.str1167[Lang.Idx]},
				{MultiInstance.REVIVE_INFO, true, true, false},
				{MultiInstance.SHOW_NOTICE_MSG, Lang.mission.str1168[Lang.Idx]},
				{MultiInstance.TARGET_INFO,Lang.mission.str1169[Lang.Idx] , 1},
		...
		[2] = {nTime = 10, nNum = 0 ,		-- 锁通过的条件（时间倒计时，杀怪个数）
			...
			tbStartEvent =
			{
				...
				{MultiInstance.TARGET_INFO, Lang.mission.str1172[Lang.Idx], 1},
			},
			tbUnLockEvent =
			{
		...
		[3] = {nTime = 0, nNum = 1 ,		-- 锁通过的条件（时间倒计时，杀怪个数）
			...
			tbStartEvent =
			{
				...
				{MultiInstance.TARGET_INFO, Lang.mission.str1173[Lang.Idx], 1},
			},
			tbUnLockEvent =
			{
			{MultiInstance.ADD_NPC, 5, 1, 2, "beida", "beida"},
			{MultiInstance.SHOW_NOTICE_MSG, Lang.mission.str1174[Lang.Idx]},
			{MultiInstance.NPC_BUBBLE, "huo1",Lang.mission.str1175[Lang.Idx], 1},
		...
		[4] = {nTime = 0, nNum = 1 ,		-- 锁通过的条件（时间倒计时，杀怪个数）
			...
			tbStartEvent =
			{
				...
				{MultiInstance.TARGET_INFO, Lang.mission.str1176[Lang.Idx], 1},
			},
			tbUnLockEvent =
			{
		...
		[5] = {nTime = 0, nNum = 2 ,		-- 锁通过的条件（时间倒计时，杀怪个数）
			...
			tbUnLockEvent =
			{
			    {MultiInstance.SHOW_NOTICE_MSG, Lang.mission.str1178[Lang.Idx]},
