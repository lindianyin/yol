...
-- 静态进度条目标
local tb	= Task:GetTarget("IconProcessStatic");
tb.szTargetName	= "IconProcessStatic";
...
function tb:Init(nNpcTempId, nMapId, nIntervalTime, szProcessInfo, szSucMsg, szFailedMsg,  tbItem, nNeedCount, szDynamicDesc, szStaticDesc, szBeforePop, szLaterPop)--
	self.nNpcTempId			= Task.ICON_PROCESS_ID;
...
function tb:SelectOpenBox()
	...
	ProgressBar:StartProcessOnPlayer(self.me, self.szProcessInfo, self.nIntervalTime, {self.OnProgressFull, self}, {self.CancelProgress, self}, tbBreakEvent)
end;
