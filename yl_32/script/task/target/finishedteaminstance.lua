-------------------------------------------------------
-- 文件名　：finishedteamlevel.lua
-- 文件描述：完成具体难度的特定多人副本
-- 创建者　：zhangzhixiong
-- 创建时间：2014-3-7
-------------------------------------------------------

local tb	= Task:GetTarget("FinishedTeamInstance");
tb.szTargetName	= "FinishedTeamInstance";
tb.REFRESH_FRAME	= 18;	-- 一秒检测一次

function tb:Init(szStaticDesc, szDynamicDesc, nMapId, nInstanceType)
	self.szStaticDesc	= szStaticDesc;
	self.szDynamicDesc	= szDynamicDesc;
	self.nMapId = nMapId;
	self.nInstanceType = nInstanceType
end

function tb:Start()
	self.bDone = 1;
	local nMapPassed = KInstance.IsMultiMapPassed(self.me, self.nMapId)
	if nMapPassed == 0 then
	    self.bDone = 0;
	end
